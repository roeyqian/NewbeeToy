...
| `config/general.toml` | 文本 TOML 配置，保存文件夹样式模块的分组路径列表。                                    |
| `config/system.toml`  | 文本 TOML 配置，保存系统环境变量模块的预设快照。                                     |
| `assets/lang/*.toml` | 运行时语言表。程序只读取对应 TOML，不会自动生成或写入翻译文件。                             |
| `assets/fonts/*.otf` | 运行时字体资源。`icon.ico` 和 `icon.png` 会直接封装进 exe。                         |

所有应用配置均为可直接编辑的 TOML 文本。构建脚本会在运行目录中缺少配置时生成默认的 `general.toml` 和 `system.toml`。
...
| `config/general.toml` | Text TOML configuration — stores folder-style group path lists.                                                                      |
| `config/system.toml`  | Text TOML configuration — stores system environment variable preset snapshots.                                                       |
...
All application configuration is directly editable TOML text. When configuration is absent, the build script creates default `general.toml` and `system.toml` files in the runtime directory.
...
| `config/general.toml` | テキスト TOML 設定 — フォルダスタイルモジュールのグループ別パス一覧を保存。                                                |
| `config/system.toml`  | テキスト TOML 設定 — システム環境変数モジュールのプリセットスナップショットを保存。                                             |
...
すべてのアプリケーション設定は、直接編集できる TOML テキストです。設定が存在しない場合、ビルドスクリプトは実行ディレクトリに既定の `general.toml` と `system.toml` を作成します。
...
| `config/general.toml` | Configuración TOML de texto — almacena las listas de rutas por grupo del módulo de estilo de carpetas.                                                                 |
| `config/system.toml`  | Configuración TOML de texto — almacena instantáneas preestablecidas de variables de entorno del sistema.                                                               |
...
Toda la configuración de la aplicación es texto TOML editable directamente. Cuando falta la configuración, el script de compilación crea archivos predeterminados `general.toml` y `system.toml` en el directorio de ejecución.
