...
fn set_folderstyle_preset_manager_entries(
    manager: &PresetManagerWindow,
    path: &Path,
) -> Result<(), String> {
    manager.set_presets(ModelRc::new(VecModel::from(folderstyle_preset_entries(
        path,
    )?)));
    Ok(())
}

fn show_folderstyle_preset_manager(
    ui: &MainWindow,
    preview_state: &Rc<RefCell<FolderStylePreviewState>>,
    folderstyle_path: &Path,
) {
    ...
    if let Err(err) = set_folderstyle_preset_manager_entries(&manager, folderstyle_path) {
...

    {
        ...
        let folderstyle_path = folderstyle_path.to_path_buf();
...

    {
        ...
        manager.on_delete_request(move |preset_name| {
            ...
            match delete_folderstyle_preset(
                &folderstyle_path,
                &preset_name,
                ui.get_language_index(),
            ) {
                Ok(()) => {
                    if ui.get_folderstyle_preset_name().as_str() == preset_name {
                        ui.set_folderstyle_preset_name("".into());
                    }
                    if let Err(err) =
                        set_folderstyle_preset_manager_entries(&manager, &folderstyle_path)
                    {
                        append_folderstyle_status_log(&ui, "ERROR", &err);
                    }
                    append_folderstyle_status_log(
                        ...
                        &tf(
                            ui.get_language_index(),
                            "folderstyle.msg.preset_deleted",
                            &[("name", &preset_name)],
                        ),
                    );
                }
                Err(err) => append_folderstyle_status_log(
                    ...
                    &tf(
                        ui.get_language_index(),
                        "folderstyle.msg.preset_delete_failed",
                        &[("error", &err)],
                    ),
                ),
            }
...
pub fn setup_folderstyle_handlers(ui: &MainWindow, app_dir: &Path) {
...

    {
        ...
        let preview_state = Rc::clone(&preview_state);
        let folderstyle_path = folderstyle_path.clone();
        ui.on_folderstyle_show_preset_manager_request(move || {
            let Some(ui) = ui_handle.upgrade() else {
                return;
            };
            show_folderstyle_preset_manager(&ui, &preview_state, &folderstyle_path);
        });
    }
